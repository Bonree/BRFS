...
public class JsonConfigProvider<T> implements Provider<Supplier<T>>
{
  ...
  {
    binder.bind(supplierKey).toProvider(of(propertyBase, clazz)).in(Singleton.class);
  ...
  {
    binder.bind(supplierKey).toProvider(of(propertyBase, clazz, defaultClass)).in(Singleton.class);
    binder.bind(instanceKey).toProvider(new SupplierProvider<>(supplierKey));
  }
