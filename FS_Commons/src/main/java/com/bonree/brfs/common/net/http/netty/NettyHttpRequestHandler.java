...
public class NettyHttpRequestHandler {
    ...
    public void requestReceived(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        if (handler == null) {
            ...
            ResponseSender
                .sendError(ctx, HttpResponseStatus.METHOD_NOT_ALLOWED, HttpResponseStatus.METHOD_NOT_ALLOWED.reasonPhrase());
            return;
        }
            ...
            public void run() {
                try {
                    if (!handler.isValidRequest(message)) {
                        LOG.error("Exception context[{}] method[{}] invalid request message[{}]", ctx.toString(),
                                  message.getPath());
                        ResponseSender
                            .sendError(ctx, HttpResponseStatus.BAD_REQUEST, HttpResponseStatus.BAD_REQUEST.reasonPhrase());
                        return;
                    }
