...
/**
 * 对{@link org.apache.curator.x.discovery.details.ServiceDiscoveryImpl<T>}重新实现
 * <p>
 * 在重新注册部分添加了对注册时间的更新
 */
    ...
    private void reRegisterServices() throws Exception {
        for (final Entry<T> entry : services.values()) {
            synchronized (entry) {
                ...
                if (oldService.getSslPort() != null) {
                    builder.sslPort(oldService.getSslPort());
                }
                entry.service = builder.build();
