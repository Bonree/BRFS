...
public class MetaTaskLeaderManager implements LeaderLatchListener {
    ...
    public void isLeader() {
        try {
            ...
            boolean startTaskPool = this.manager.startTaskPool(META_TASK_MANAGER);
            if (!startTaskPool) {
    ...
    private void sumbitTask() throws ParamsErrorException {
        ...
        SumbitTaskInterface metaJob = QuartzSimpleInfo.createCycleTaskInfo("META_MANAGER_TASK",
                                                                           config.getCreateTaskIntervalTime(), 60000, metaDataMap,
    ...
    public void createCheckCyclePool() throws ParamsErrorException {
        ...
        boolean startTaskPool = this.manager.startTaskPool(COPY_CYCLE_POOL);
        if (!startTaskPool) {
            LOG.info("Follower will quiting  !!!");
            return;
        }
        ...
        SumbitTaskInterface sumbit = QuartzCronInfo
            .getInstance("CYCLE_CHECK_JOB", "CYCLE_CHECK_JOB", this.config.getCheckCronStr(), content, CheckCycleJob.class);
        startTaskPool = this.manager.addTask("COPY_CYCLE_POOL", sumbit);
