...
import com.bonree.brfs.schedulers.utils.BRFSCheckFilter;
...
public class SystemCheckJob extends QuartzOperationStateWithZKTask {
    ...
    public TaskResultModel checkFiles(AtomTaskModel atom, DiskDaemon daemon) {
        ...
        for (LocalPartitionInfo path : daemon.getPartitions()) {
            ...
            List<BRFSPath> brfsPaths =
                BRFSFileUtil.scanBRFSFiles(path.getDataDir(), snMap, snMap.size(), new BRFSCheckFilter(startTime, endTime));
            if (brfsPaths != null) {
                for (BRFSPath brfsPath : brfsPaths) {
