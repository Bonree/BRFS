...
public class CheckCycleJob extends QuartzOperationStateTask {
    ...
    public void operation(JobExecutionContext context) throws Exception {
        ...
        LOG.info("[{}] Check the number of data copies [{} - {}]", TimeUtils.formatTimeStamp(currentTime),
                 TimeUtils.formatTimeStamp(sgraday), TimeUtils.formatTimeStamp(lgraday));
        Map<String, List<Long>> snTimes = collectionTimes(needSns, sgraday, lgraday);
        if (snTimes == null || snTimes.isEmpty()) {
            LOG.warn("{} - {} time, no data to check copy count", TimeUtils.formatTimeStamp(sgraday),
                     TimeUtils.formatTimeStamp(lgraday));
            return;
        }
        List<Map<String, Long>> times = converTimes(snTimes);
        for (Map<String, Long> sourceTimes : times) {
     ...
     * @user <a href=mailto:zhucg@bonree.com>朱成岗</a>
     */
    public void createSingleTask(MetaTaskManagerInterface release, List<StorageRegion> needSns, List<Service> services,
                                 TaskType taskType, Map<String, Long> sourceTimes) throws Exception {
