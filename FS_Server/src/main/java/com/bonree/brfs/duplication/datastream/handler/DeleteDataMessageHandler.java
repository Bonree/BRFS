...
public class DeleteDataMessageHandler implements MessageHandler {
    ...
    public DeleteDataMessageHandler(ZookeeperPaths zkPaths, ServiceManager serviceManager,
                                    StorageRegionManager storageNameManager) {
    ...
    public void handle(HttpMessage msg, HandleResultCallback callback) {
        ...
        ReturnCode code =
            checkTime(times.get(0), times.get(1), sn.getCreateTime(), Duration.parse(sn.getFilePartitionDuration()).toMillis());
    ...
    private ReturnCode checkTime(String startTimeStr, String endTimeStr, long ctime, long granule) {
        ...
        if (startTime != (startTime - startTime % granule)
            ...
            LOG.warn("starttime and endTime granule is not match !!! startTime: [{}], endTime:[{}], granue:[{}]", startTimeStr,
                     endTimeStr, granule);
        }
