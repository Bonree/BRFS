...
public class StorageRegionConfig {
    ...
    private static final int DEFAULT_REPLICATE_NUM = Configs.getConfiguration()
            .GetConfig(StorageConfigs.CONFIG_STORAGE_REGION_REPLICATE_COUNT);
    private static final String DEFAULT_DATA_TTL = Configs.getConfiguration()
            .GetConfig(StorageConfigs.CONFIG_STORAGE_REGION_DATA_TTL);
    private static final long DEFAULT_FILE_CAPACITY = Configs.getConfiguration()
            .GetConfig(StorageConfigs.CONFIG_STORAGE_REGION_FILE_CAPACITY);
    private static final String DEFAULT_FILE_PARTITION_DURATION = Configs.getConfiguration()
            .GetConfig(StorageConfigs.CONFIG_FILE_PATITION_DURATION);
    ...
    public void update(Attributes params) {
        ...
        attributes.putInt(CONFIG_REPLICATE_NUM,
                params.getInt(CONFIG_REPLICATE_NUM, attributes.getInt(CONFIG_REPLICATE_NUM)));
        attributes.putString(CONFIG_DATA_TTL, params.getString(CONFIG_DATA_TTL, attributes.getString(CONFIG_DATA_TTL)));
        attributes.putLong(CONFIG_FILE_CAPACITY,
                params.getLong(CONFIG_FILE_CAPACITY, attributes.getLong(CONFIG_FILE_CAPACITY)));
        attributes.putString(CONFIG_FILE_PARTITION_DURATION,
                params.getString(CONFIG_FILE_PARTITION_DURATION, DEFAULT_FILE_PARTITION_DURATION));
    }
