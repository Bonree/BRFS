...
public class DefaultFileObjectSyncProcessor implements FileObjectSyncProcessor {
    ...
    private List<FileObjectSyncState> getFileStateList(FileNode fileNode) {
        ...
        for (DuplicateNode node : fileNode.getDuplicateNodes()) {
            ...
            if (connection == null || connection.getClient() == null) {
                LOG.error("duplication node[{}, {}] of [{}] is not available, that's maybe a trouble!", node.getGroup(),
                          node.getId(), fileNode.getName());
                continue;
            }
            ...
            if (fileLength < 0) {
                LOG.error("duplication node[{}, {}] of [{}] can not get file length, that's maybe a trouble!", node.getGroup(),
                          node.getId(), fileNode.getName());
                continue;
            }
    ...
    private boolean doSynchronize(FileNode fileNode, long correctLength, List<FileObjectSyncState> lacks,
                                  List<FileObjectSyncState> fulls) {
        ...
        for (FileObjectSyncState state : lacks) {
            ...
            LOG.info("start synchronize file[{}] at data node[{}, {}]", fileNode.getName(), state.getServiceGroup(),
                     state.getServiceId());
            if (!client.recover(state.getFilePath(), state.getFileLength(), fullStates)) {
                LOG.error("can not synchronize file[{}] at data node[{}, {}]", fileNode.getName(), state.getServiceGroup(),
                          state.getServiceId());
                allSynced = false;
            }
