...
public class testAvailServer {
	...
	public static void testInit() {
		...
		try {
			String serviceId = "server2";
			configuration.parse("E:/tmp/server_default.properties");
			configuration.printConfigDetail();
			configuration.setProperty(ResourceTaskConfig.RESOURCE_FRAMEWORK_SWITCH, "false");
			...
			ServiceManager sm = new DefaultServiceManager(client.usingNamespace(clusterName));
			sm.start();
			...
			StorageNameManager snm = new DefaultStorageNameManager(client);
			snm.start();
			InitTaskManager.initManager(configuration, sm,snm,clusterName,serviceId,clusterName);
