    ...
    public void childEvent(CuratorFramework client, TreeCacheEvent event) throws Exception {
        ...
        if (event.getType().equals(TreeCacheEvent.Type.NODE_ADDED)
            ...
            List<String> splitPaths =
                Lists.newArrayList(Splitter.on(Constants.SEPARATOR).omitEmptyStrings().trimResults().split(path));
            ...
            if (endStr.length() > 16 && splitPaths.size() > 4) {
                if (splitPaths.contains(Constants.VIRTUAL_ROUTE)) {
                    VirtualRoute route =
                        JSON.parseObject(BrStringUtils.fromUtf8Bytes(event.getData().getData()), VirtualRoute.class);
                    routeRoleCache.getVirtualRouteCache().put(route.getVirtualID(), route);
                } else if (splitPaths.contains(Constants.NORMAL_ROUTE)) {
                    NormalRoute route =
                        JSON.parseObject(BrStringUtils.fromUtf8Bytes(event.getData().getData()), NormalRoute.class);
